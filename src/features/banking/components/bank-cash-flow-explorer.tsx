      ...
      <div className='space-y-6'>
        ...
        {isPending ? (
          ...
          <>
            ...
            <Card data-testid='cash-flow-period-detail'>
              <CardHeader className='pb-4'>
                <CardTitle className='text-base'>Period detail</CardTitle>
                <p className='text-sm text-muted-foreground'>
                  Exact values from the selected currency. Select a period to inspect its
                  transactions.
                </p>
              </CardHeader>
              <CardContent className='p-0'>
                <div className='overflow-x-auto'>
                  <table className='w-full table-fixed text-sm sm:min-w-[42rem]'>
                    <caption className='sr-only'>Cash-flow values by period</caption>
                    <thead className='border-y bg-muted/30 text-left text-xs text-muted-foreground'>
                      <tr>
                        <th className='w-[28%] px-2 py-3 font-medium sm:w-auto sm:px-5 sm:py-3'>
                          Period
                        </th>
                        <th className='w-[18%] px-1 py-3 text-right font-medium sm:w-auto sm:px-3'>
                          Money in
                        </th>
                        <th className='w-[18%] px-1 py-3 text-right font-medium sm:w-auto sm:px-3'>
                          Money out
                        ...
                        <th className='w-[18%] px-2 py-3 text-right font-medium sm:w-auto sm:px-5 sm:py-3'>
                          Transactions
                        </th>
                      </tr>
                    </thead>
                    <tbody className='divide-y'>
                      {selectedSeries.buckets.map((bucket: BankCashFlowBucket) => (
                        <tr
                          key={bucket.startDate}
                          className={cn(
                            selectedBucket?.startDate === bucket.startDate && 'bg-muted/30',
                          )}
                        >
                          <th
                            scope='row'
                            className='px-2 py-3 text-left text-xs font-medium sm:px-5 sm:py-3 sm:text-sm'
                          >
                            <button
                              type='button'
                              className='underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring'
                              aria-current={
                                selectedBucket?.startDate === bucket.startDate ? 'true' : undefined
                              }
                          ...
                          <td className='whitespace-nowrap px-1 py-3 text-right font-mono text-xs text-success sm:px-3 sm:py-3 sm:text-sm'>
                            {formatAmount(bucket.income, selectedSeries.currency)}
                          </td>
                          <td className='whitespace-nowrap px-1 py-3 text-right font-mono text-xs text-clay sm:px-3 sm:py-3 sm:text-sm'>
                            {formatAmount(bucket.expenses, selectedSeries.currency)}
                          </td>
                          <td className='whitespace-nowrap px-1 py-3 text-right font-mono text-xs sm:px-3 sm:py-3 sm:text-sm'>
                            {formatAmount(bucket.net, selectedSeries.currency)}
                          </td>
                          <td className='px-2 py-3 text-right font-mono text-xs sm:px-5 sm:py-3 sm:text-sm'>
